---
description: 'Comprehensive guidelines for creating custom agent files for GitHub Copilot'
applyTo: '**/*.agent.md'
---
...
- **Currently not supported**; do not include this property as it will be ignored
...
#### **infer** (DEPRECATED — DO NOT USE)
- **Retired.** Use `user-invocable` and `disable-model-invocation` instead.
- If both `infer` and `disable-model-invocation` are set, `disable-model-invocation` takes precedence.
- Existing agents with `infer: false` should migrate to `disable-model-invocation: true`.

#### **user-invocable** (OPTIONAL)
- Boolean controlling whether users can directly invoke this agent from the agent picker
- Default: `true` if omitted
- Set to `false` for subagents that should only be invoked via orchestration

#### **disable-model-invocation** (OPTIONAL)
- Boolean preventing Copilot from automatically invoking this agent as a subagent
- Default: `false` if omitted
- Takes precedence over deprecated `infer` property
...
- [ ] `model` specified if task needs a specific AI model (string or fallback array)
- [ ] `target` set if environment-specific
- [ ] `user-invocable` / `disable-model-invocation` set correctly for invocation control
