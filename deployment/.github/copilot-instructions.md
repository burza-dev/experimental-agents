...
This project uses specialized agents for TDD-driven development. See `AGENTS.md` for the complete workflow contract.
...
| `test-specialist` | Writes unit/integration tests first |
...
2. **Test** → Test Specialist writes failing tests first
3. **Implement** → Implementer writes code to pass tests
4. **Iterate** → Repeat test/implement until quality gates pass
5. **Review** → Reviewers provide feedback (no diffs)
6. **Validate** → Manager confirms all gates pass
...
7. **Listen to user request** - user requests must be adressed, even if they seem to be out of the task scope
...
# Check complexity
uv run radon cc src/ tests/ -a -nb
uv run radon mi src/ tests/ -nb

# Verify coverage is at least 75%
