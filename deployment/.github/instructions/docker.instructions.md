...
In Dockerfile, handle platform differences:
...
1. **scratch** - empty image, for static Go/Rust binaries
2. **Distroless** - minimal Google images, no shell
3. **Chainguard** - hardened images with SBOMs, updated daily
4. **Alpine** - minimal with musl libc, ~5MB
5. **Slim variants** - Debian-based but reduced, ~50-80MB
...
# Build with cache
RUN --mount=type=cache,target=/build/CMakeCache \
...
**Dynamic linking** - smaller image, shared libraries:
...
COPY . /src
RUN cmake -B /src/build -S /src && cmake --build /src/build
...
Use official toolchain images:

| Language | Image | Use Case |
|----------|-------|----------|
| GCC | `gcc:14` | GNU toolchain |
| Clang | `silkeh/clang:18` | LLVM toolchain |
| Rust | `rust:1.78` | Rust projects |
| Go | `golang:1.22` | Go projects |
